...
pub fn sort_by_name_main(file_type : &str, dest : String) {
    ...
    // let dest = String::from("temp_folder");
    // let file_type = String::from("ch4");
    let mut count = 0;
    let mut flag = true;
    ...
    if count == 0 && flag == true {
        println!("There are no matching files with given string 😐");
        println!("Quitting... 🚫");
        return;
    }
}
...
pub fn metadata_and_print(fn_path: PathBuf, dest: &str, file_type: &str, count: i32) -> i32{
        ...
        if file_name.contains(&file_type) && metadata.is_file() {
